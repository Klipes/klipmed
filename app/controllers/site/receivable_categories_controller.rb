class Site::ReceivableCategoriesController < ApplicationController
  ...
  def index
    @receivable_categories = ReceivableCategory.not_deleted.company(current_user.company_id).order(:description).page params[:page]
  end
  ...
  def destroy
    @receivable_category.update(deleted_at: DateTime.now)
  end
